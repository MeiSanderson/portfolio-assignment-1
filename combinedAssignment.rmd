...
for (s in 1:simulations) {
  N <- 10
  female <- rbinom(N, 1, 0.49)  #MNE - female = 1, male = 0
  weight <-
    ifelse(female == 1, rnorm(N, 4.96, 0.20), rnorm(N, 5.13, 0.17)) # MNE - female = 1 aka it's a female then the simulation is rnorm(N, 4.96, 0.20). If female isn't equal to 1, it gives the simulation for men rnorm(N, 5.13, 0.17)
  
  sum_weight[s] <- sum(exp(weight)) # MNE - exp() is cause the weight is a log value, so we take exp() to remove that
}
...
elevator <- 1750
probability <- 1 - pnorm(elevator, mean(sum_weight), sd(sum_weight)) # MNE - pnorm = calculates the probability of something happening under the normal distribution (0-1) - the probability of it being higher than the elevator value given the mean and sd
...
# viewing total savings
head(savings_CostxSize)


hist(savings_CostxSize)
...
# Generate random draws for the cost difference
effectiveness_difference <- rnorm(num_simulations, mean = 3.0, sd = 1.0)

# Create scatterplot
plot(cost_difference, effectiveness_difference, 
